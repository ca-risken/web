    ...
    methods: {
        async listOSINT(project_id) {
            const res = await this.$axios.get(
              '/osint/list-osint/?project_id=' + project_id
            ).catch((err) =>  {
        ...
        async getOSINT(project_id,osint_id) {
            const res = await this.$axios.get(
                '/osint/get-osint/?project_id=' + project_id +
                '&osint_id=' + osint_id
            ).catch((err) =>  {
              return Promise.reject(err)
            })
        ...
        async putOSINT(project_id,osint_id,resource_type,resource_name) {
            ...
            const res = await this.$axios.post('/osint/put-osint/', param).catch((err) =>  {
              return Promise.reject(err)
            })
            return res.data.data
        },
        async listOSINTDataSource(project_id) {
            const res = await this.$axios.get(
              '/osint/list-datasource/?project_id=' + project_id
            ).catch((err) =>  {
              return Promise.reject(err)
            })
            return res.data.data
        ...
        async AttachReferenceDataSource(project_id,osint_id,resource_type){
            ...
            for (var i = 0; i < dataSources.length; i++){
                ...
                var existRelOsintDataSource = await this.CheckExistRelOSINTDataSource(project_id,dataSource.osint_data_source_id,osint_id)
                if (this.matchResourceType(dataSource.name,resource_type) && !existRelOsintDataSource ){
                    await this.attachRelOSINTDataSource(project_id,0,dataSource.osint_data_source_id,osint_id,0, true)
                }
            }
        },
        matchResourceType(data_source_name,resource_type){
            ...
            var dataSourceArr = this.DataSourceReferenceResourceType.get(resource_type)
            if (dataSourceArr.includes(data_source_name)){
                return true
            }
            return false
        },
        async CheckExistRelOSINTDataSource(project_id,osint_data_source_id,osint_id) {
            var res = await this.listRelOSINTDataSource(project_id,osint_data_source_id,osint_id).catch((err) =>  {
                return Promise.reject(err)
              })
            if (!res.rel_osint_data_source || res.rel_osint_data_source.length() < 1) {
                return false
            }
            return true
        },
        async attachRelOSINTDataSource(project_id, rel_osint_data_source_id,osint_data_source_id,osint_id,scan_at,isNew){
            const ret = await this.putRelOSINTDataSource(project_id, rel_osint_data_source_id,osint_data_source_id,osint_id)
              ...
              if ( isNew && updated.rel_osint_data_source_id ){
                this.detectWordDefault.forEach(async word => {
                  await this.putDetectWord(project_id,updated.rel_osint_data_source_id, word)
                })
              }
        },
        async putRelOSINTDataSource(project_id, rel_osint_data_source_id,osint_data_source_id,osint_id,scan_at) {
            const param = {
                project_id: project_id,
        ...
        async listDetectWord(project_id,rel_osint_data_source_id) {
            const res = await this.$axios.get(
                '/osint/list-word/?project_id=' + project_id +
                '&rel_osint_data_source_id=' + rel_osint_data_source_id
              ).catch((err) =>  {
                return Promise.reject(err)
              })
        ...
        async putDetectWord(project_id, rel_osint_data_source_id,word) {
            ...
            const res = await this.$axios.post('/osint/put-word/', param).catch((err) =>  {
                return Promise.reject(err)
              })
            return res.data.data
        },
        ...
        async deleteDetectWordByRelOsintDataSourceID(project_id, rel_osint_data_source_id) {
            const resListWord = await this.$axios.get('/osint/list-word/?project_id=' + project_id +
                '&rel_osint_data_source_id=' + rel_osint_data_source_id
            ).catch((err) =>  {
                return Promise.reject(err)
              })
