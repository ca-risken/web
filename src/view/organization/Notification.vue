...
export default {
  ...
  methods: {
    ...
    async refleshList() {
      this.loading = true
      this.clearList()
      const notification = await this.listOrganizationAlertNotification().catch(
        (err) => {
    ...
    // delete
    async deleteItem() {
      await this.deleteOrganizationAlertNotification(
    ...
    async testNotification() {
      await this.testOrganizationAlertNotification(
        this.dataModel.notification_id
      ).catch((err) => {
        this.finishError(err.response.data)
        return Promise.reject(err)
    ...
    async putItem() {
      ...
      await this.putOrganizationAlertNotification(param).catch((err) => {
        this.finishError(err.response.data)
        return Promise.reject(err)
      })
