              ...
              <v-list-item two-line>
                <v-list-item-content>
                  <v-list-item-title class="headline">
                    ...
                    <v-chip
                      dark
                      :color="getDataSourceStatusColor(jiraModel.status)"
                    >
      ...
      <v-card>
        ...
        <v-list two-line>
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title
                v-text="diagnosisModel.jira_setting_id"
              ></v-list-item-title>
...
export default {
        ...
        identity_field: {
          ...
          validator: [
            (v) =>
              !v ||
              v.length <= 50 ||
              'identity_field must be less than 50 characters',
          ],
        },
        identity_value: {
          ...
          validator: [
            (v) =>
              !v ||
              v.length <= 50 ||
              'identity_value must be less than 50 characters',
          ],
        },
        jira_id: {
          label: 'JIRA ID(JIRA Project ID)',
          placeholder: '-',
          validator: [
            (v) =>
              !v || v.length <= 50 || 'JIRA ID must be less than 50 characters',
          ],
        },
        jira_key: {
          label: 'JIRA Project',
          ...
          validator: [
            ...
            (v) =>
              !v ||
              v.length <= 50 ||
              'JIRA Project must be less than 50 characters',
          ],
        },
      },
      diagnosisModel: {
  ...
  computed: {
    headers() {
      return [
        ...
        {
          text: this.$i18n.t('item["JIRA Project"]'),
          align: 'start',
          sortable: true,
          value: 'jira_key',
        },
  ...
  methods: {
      ...
      const res = await this.$axios
        .get(
          '/diagnosis/list-jira-setting/' +
            '?diagnosis_data_source_id=' +
            this.diagnosisModel.diagnosis_data_source_id +
            '&project_id=' +
            this.$store.state.project.project_id
        )
        .catch((err) => {
    ...
    async deleteItem() {
      ...
      await this.$axios
        .post('/diagnosis/delete-jira-setting/', param)
        .catch((err) => {
          this.finishError(err.response.data)
          return Promise.reject(err)
        })
    ...
    async putItem() {
      ...
      await this.$axios
        .post('/diagnosis/put-jira-setting/', param)
        .catch((err) => {
          this.finishError(err.response.data)
          return Promise.reject(err)
        })
